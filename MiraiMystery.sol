...
contract MiraiMystery is Ownable {
    ...
    function listBalance() public view returns(uint256) {
        return nft.balanceOf(thisContract);
    }
    
    function ownerOfId(uint256 _tokenId) public view returns(address) {
    ...
    function reload(uint256[] calldata _ids, uint256[] calldata _dropRates) public onlyOwner {
        ...
        require(_ids.length == _dropRates.length);
        for(uint256 i = 0; i < _ids.length; i++) {
            nftID.push(_ids[i]);
            nftDropRate.push(_dropRates[i]);
        }
    }
